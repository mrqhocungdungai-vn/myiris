...
description: >
  Use when a decision needs its trade-offs named before code is written — where a
  seam belongs, whether two things are one concept or two, what a module should own
  versus expose, or which of several structures survives the next change. Best
  suited to the design stage of an OpenSpec change, when a proposal exists and the
  approach is still open.
  Do NOT use to review code that already exists — that is `code-reviewer`. Do NOT
  use for questions with a single correct answer discoverable by reading this repo;
  it reasons about structure and will not out-read the files. It has no editing
  tools by design: its output is an argument, not a change.
