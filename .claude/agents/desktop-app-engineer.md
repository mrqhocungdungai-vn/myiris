...
description: >
  Use for questions about the Electron process boundary and the macOS packaging
  path: the main/renderer split, preload and context isolation, what may cross IPC,
  process-group lifetime and teardown, permission handlers, code signing and
  notarization, and packaging or `extraResources` layout. This is the stack this
  repo actually ships, so prefer it over a general answer when the question touches
  any of those.
  Do NOT use for renderer feature work, React state, or WebGL/rendering
  performance — those are ordinary app code and not about the process boundary. Do
  NOT use for Tauri: the body covers it, this repo does not use it, and an answer
  framed around Tauri would not apply.
