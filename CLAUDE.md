...
| Gesture/hand control (MediaPipe config, gesture→action mapping), the eye HUD sharing that camera session — **its readout reports the real host** (CPU/GPU/network, sampled only while the camera is on) **and the app's own token spend**, plus the badge that announces a finished run beside the ring for a few seconds and is never replayed — the **activity strip** along the frame's bottom and why its depth is decided by the build mode alone, and HUD mode's camera-zoom toggle | [docs/GESTURES.md](docs/GESTURES.md) |
| **What each engine costs, in tokens** — the one unit both engines actually report, so nothing is estimated from a price table. Two accounts that are never combined, counted from app start and **never gated on the camera** (only the display is), monotone across a Live reconnect whether the API's figures are cumulative or per-message, and read by nothing in the app: no prompt, no verb, no spoken answer | `openspec/specs/token-accounting/spec.md` + `electron/token-ledger.mjs` |
...
| **The second brain, and the galaxy it is drawn as** — two capabilities, deliberately: `second-brain-layer` is **the feature** (the "show second brain" toggle and its exclusivity with the drawing canvas, HUD-only lifetime, main's ownership of the vault graph, the note reader, containment of untrusted note content, gating on the vault existing), `galaxy-view` is **how a link-graph is drawn and flown** and names no note (backdrop, the pointed-at link cluster, node labels, the camera anchor, the sight, the grab affordance). A rule belongs to whichever survives the galaxy being handed a different data source | `openspec/specs/second-brain-layer/spec.md` + `openspec/specs/galaxy-view/spec.md` |
| **Listen-only mode** — Iris goes silent so someone else can ask the user a question, and answers about it afterwards (complete silence, system-audio capture mixed with the mic, main-process ownership) and its **bounded listening window**: engaging opens an absolute deadline that nothing Iris hears extends, and at that deadline the mode disengages itself through the same writer the user's toggle uses. **Nothing is retained** — the voice session's own audio context is what the user asks against | [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#listen-only-mode--hearing-a-question-you-will-be-asked-about) + `openspec/specs/listen-only-mode/spec.md` |
| **Prepared answers** — the moment listening ends, Iris looks in **the folder the user has open** for the answer they already wrote, and reads it back **in their words**. A worker-free tool (`find_prepared_answer`): no run, no tokens, no credential, and outside `PIPELINE_ONLY_TOOLS`. Two properties carry it — the app returns *material* and the model that heard the question does the matching (local scoring only on overflow, and the narrowing is stated), and a found answer is **announced in one line, then read only on the user's cue** | `openspec/specs/prepared-answers/spec.md` + [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#the-prepared-answer--what-happens-the-moment-listening-ends) |
