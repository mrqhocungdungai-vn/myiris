...
- **Iris never reads or writes the user's `~/.claude`.** This takes *two* mechanisms, and `settingSources` is only one of them. Personas go to the SDK by value, skills/commands come from the bundled plugin, and `settingSources` excludes the `user` scope (keeping `project`, so a run still picks up the repo it works in). But several inputs are read and written **regardless of `settingSources`** — the session transcript of every run, the always-read `.claude.json`, auto-memory — and they all live under `CLAUDE_CONFIG_DIR`. So `worker-env.mjs` pins that to `~/.iris/claude-home` (plus `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1`) for both roles; without it a run writes its transcript straight into the user's `~/.claude/projects/`. A consequence worth knowing: this also cuts off the *host* Claude Code login, so a credential must come from the environment — see [docs/PIPELINE_INTERNALS.md](docs/PIPELINE_INTERNALS.md). The only code that touches `~/.claude` is the Settings-panel action that removes what *older* Iris versions left there, and only on an explicit click.
