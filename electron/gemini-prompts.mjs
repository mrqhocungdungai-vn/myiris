  ...
  function buildSystemInstructionText() {
    ...
    if (pipelineAvailable) {
      lines.push(
        ...
        "LIVE QUESTIONS — when a resident session (shaping, or working on a note) reaches a real fork in the road MID-TASK — including a destructive edit to a note that needs confirming before it's applied — it pauses immediately and you receive SYSTEM_EVENT_PO_QUESTION with a list of questions and options. Read each one aloud right then — don't wait for the run to finish, it hasn't. Once you have every answer, call answer_claude_question with the exact question text and the chosen option's label for each; the run resumes the instant you do. If the user asks what you'd pick, suggest the first-listed option, but always submit what they actually chose.",
