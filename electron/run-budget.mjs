// Per-role turn and spend ceilings for every Claude run.
//
...
//
//   PO,  one turn: grill → full 4-artifact OpenSpec proposal   28 turns  $0.97
//   DEV, one run:  implement every task in that change, TDD    29 turns  $0.78
//
...
// overturned: PO is *not* the cheaper role. It takes as many turns as DEV and
// spends more, because a propose turn writes and validates four artifacts. The
// two ceilings are therefore near-symmetric, PO's slightly higher on spend
// because its default model (Opus 5) is the pricier one.
//
// Electron-free, no I/O; `env` is always an argument.

/** @typedef {"po" | "dev" | "plain"} BudgetRole */

export const DEFAULT_RUN_BUDGETS = Object.freeze({
  po: Object.freeze({ maxTurns: 150, maxBudgetUsd: 6 }),
  dev: Object.freeze({ maxTurns: 150, maxBudgetUsd: 5 }),
  // Plain Claude answers a voice request rather than implementing a change, so
  // it has no reason to run as long as either role.
  plain: Object.freeze({ maxTurns: 60, maxBudgetUsd: 2 }),
});
...
/**
 * The ceilings a run of this role starts with. The env overrides are global on
 * purpose: they exist so a user who hits a ceiling can raise it in one place
 * without having to know which role was running.
 * @param {BudgetRole} role
 * @param {Record<string, string | undefined>} [env]
 * @returns {{ maxTurns: number, maxBudgetUsd: number }}
 */
export function resolveRunBudget(role, env = process.env) {
  const defaults = DEFAULT_RUN_BUDGETS[role] ?? DEFAULT_RUN_BUDGETS.plain;
  return {
