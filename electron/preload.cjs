...
contextBridge.exposeInMainWorld("iris", {
  ...
  answerClaudeQuestion: (answers) => ipcRenderer.invoke("claude:answer-question", answers),
  ...
  saveClaudeToken: (token, key) => ipcRenderer.invoke("config:save-claude-token", { token, key }),
  removeClaudeToken: (key) => ipcRenderer.invoke("config:remove-claude-token", { key }),
