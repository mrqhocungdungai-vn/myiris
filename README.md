...
**You never pick a mode or a role.** Iris reaches Claude through seven named tools — Shape, Canvas, Note, Build, Finish, Look, Notes — and chooses one per request from what you said and what your project looks like. Behind each one is a full Claude Code agent with its own model, its own bounded set of skills, and its own ceilings. Three of them are **stateful** (a resident session that can pause mid-turn to ask you something by voice); the other four are **stateless** (one `query()` per run — normally listing decisions at the end instead of asking, though Build with no open change may ask). The pipeline uses [mattpocock/skills](https://github.com/mattpocock/skills), especially **Grill Me** when shaping a request, and [Fission-AI/openspec](https://github.com/Fission-AI/openspec) for **SDD (spec-driven development)**: shaping grills the request into a proper spec, and building implements it via **`opsx:apply`**. See "Claude pipeline" below for how it's detected and enabled.
...
# IRIS_CLAUDE_QUESTION_TIMEOUT_MS=300000
...
**Listen-only mode** — Iris goes completely silent and starts hearing the audio
your Mac plays as well as the room, so someone else can ask you something — in
the room or on a call — and you can ask her about it right afterwards. Nothing
is saved anywhere: what she heard stays in the voice conversation, which is
what you ask against. The mode **ends on its own** after five minutes
(`IRIS_LISTEN_MAX_MINUTES`, up to 15), with the time remaining shown on screen
since she cannot warn you by voice. No reconnect on either transition. It has
the same three-surface pattern: a headphone button beside the mic-mute control
(deck and HUD), a tray item, and a global hotkey, `⌥L` by default
(`IRIS_LISTEN_HOTKEY` to change it). It's independent of the mic — mute the
mic and Iris still hears your machine — and resets to audible on every wake.
...
[docs/PIPELINE_GUIDE.md](docs/PIPELINE_GUIDE.md#3b-listen-only-mode).
