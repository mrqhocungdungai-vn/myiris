...
A pre-dispatch, slot-independent gate that parks a Gemini-authored task brief for the user to Approve, Edit, or Cancel before any Claude tokens are spent — so a wrong or incomplete brief costs nothing to discover. Sits entirely before `submit_claude_task` dispatches into `run-execution-queue`; the queue, the resident PO session, and DEV's spawn mechanism are untouched.
