...
The renderer SHALL NOT use blocking synchronous dialogs (`window.confirm`, `window.alert`, `window.prompt`) for in-app interactions. Such dialogs halt the entire renderer event loop until dismissed — freezing the orb render loop, stalling the 24 kHz audio playback schedule, and pausing gesture tracking. Should an in-app confirmation ever be needed, it SHALL be non-blocking, so the render loop, the audio schedule, and gesture tracking all keep running while it is open.
