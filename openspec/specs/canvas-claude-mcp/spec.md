...
An Iris-hosted local MCP server that connects Claude to the `hud-drawing-canvas` whiteboard: Claude reads the current canvas fresh on call and draws on it (create/update/delete elements), Iris-scoped per run for both the PO Agent SDK session and DEV/plain CLI runs, gated on Claude availability and on the canvas having been engaged this session so a pure-voice session pays no token cost.
