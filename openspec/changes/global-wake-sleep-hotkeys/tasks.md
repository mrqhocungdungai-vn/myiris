...
- [ ] 1.1 Pick the wake and sleep chords. Check each against macOS system shortcuts and
      ...
      degrades gracefully
- [ ] 1.2 Add `IRIS_WAKE_HOTKEY` and `IRIS_SLEEP_HOTKEY` to `.env.example` following the
      existing two-block pattern at lines 61-71; CLAUDE.md names that file the
      authoritative list

## 2. Register them globally

- [ ] 2.1 Add the two accessors to **`electron/window.mjs`**, next to `hudHotkey()` and
...
- [ ] 2.2 Thread them out through `electron/wiring-live.mjs` (deps at ~149-150, return
      at ~160-161) and `electron/wiring.mjs:421-422`
- [ ] 2.3 **Update the exact-key-set assertion in `electron/wiring-live.test.mjs`** — it
      asserts the sorted key list of the live wiring's return, so two new accessors fail
      it until listed
- [ ] 2.4 Register both in `electron/main.mjs` next to the existing registrations
...
- [ ] 2.5 Handle the no-window case for wake: `emitToRenderer` returns silently when
...
- [ ] 2.6 Wrap `globalShortcut.register` in try/catch — a malformed accelerator can
...
- [ ] 2.7 Confirm teardown releases them — `globalShortcut.unregisterAll()` is bound to
...
- [ ] 3.1 Add the two hotkey values to `getFullConfig()`'s return in
      `electron/user-config.mjs`. They are display values, not credentials, so unlike
      the API keys they can be returned as-is
- [ ] 3.2 Add them to the `IrisConfig` type in `src/vite-env.d.ts`
- [ ] 3.3 Extend `wakeCaption()`'s input type in `src/lib/wake-caption.ts` and thread the
      value through App.tsx's call, so the caption follows configuration instead of a
      hardcoded literal
- [ ] 3.4 Add `src/lib/accelerator-label.ts` + test: parse an Electron accelerator
      ...
      component
- [ ] 3.5 Update `src/components/SetupPanel.tsx`, which hardcodes "Press W any time to
      wake, S to sleep."

## 4. Update every place the keys are named

- [ ] 4.1 Update the asleep prompt, captions, tooltips, and setup guidance to the new
      chords, driven by the configured value from task 3
- [ ] 4.2 Render the chord as separate keycaps using the helper from 3.4
- [ ] 4.3 **Update `src/lib/wake-caption.test.ts`** — four assertions hardcode "press W"
      / "Press W to wake Iris" and all four break
- [ ] 4.4 Update the README section that documents the two existing hotkeys by name, so
      all four are listed together
- [ ] 4.5 Grep for the old bare-key wording to catch copies missed above

## 5. Remove the renderer handler

- [ ] 5.1 Delete the `keydown` handler at `src/App.tsx:796-813` — the whole `useEffect`
...
- [ ] 5.2 Do this **after** task 4, not before: in between, the UI would tell the user to
...
- [ ] 5.3 Checked and expected to find nothing: no remaining renderer key handler matches
      an Option chord on `event.key` (all match `Escape` or digits). Keep the `event.code`
      convention noted for future handlers

## 6. Verify

- [ ] 6.1 Run the five gates: `npm run build`, `npm test`, `npm run lint`,
