...
- [x] 4.4c An act is suppressed when the worker's own prose covered the same moment. An assistant message carries prose and a tool call TOGETHER — "let me add three boxes" then add_elements — so both paths fired and the user heard one thing twice, the second time in worse words. Acts cover silence; prose means there is none. Scoped per run, since two conversations can now be live at once
