...
- [ ] 1.1 **Enable private vulnerability reporting** on `mrqhocungdungai-vn/myiris`. It
...
- [ ] 1.2 Fix `package.json`'s `bugs.url`, which points at an issue tracker that is
      disabled on this repo. Either enable Issues or point it somewhere real
- [ ] 1.3 State what a reporter can expect: acknowledgement timeframe, and that there are
      no bounties. Do not promise a response time that will not be met
- [ ] 1.4 State which versions are covered. There are no tags and no releases, so say
      plainly that `main` is the only supported line rather than inventing a version
      table

## 2. State the boundaries accurately

- [ ] 2.1 Permission posture: `bypassPermissions` is the intentional default for the
...
- [ ] 2.2 Honour the standing instruction in `electron/run-hooks.mjs`: a determined or
...
- [ ] 2.3 Review gate — state the shape, not just the existence. Give the per-verb park
...
- [ ] 2.4 Say the gate is user-configurable and can be turned off entirely
...
- [ ] 2.5 Spoken audio is untrusted input, and it reaches an agent that executes tools.
      Say it directly
- [ ] 2.6 Credentials: `GEMINI_API_KEY` is always withheld from the worker;
...
- [ ] 2.7 `~/.claude` is never read or written, and it takes **both** `settingSources`
...
- [ ] 2.8 Renderer: the production CSP in full, and the fact that `'unsafe-inline'`
...
- [ ] 2.9 Secret scanning is a gate that fails closed without `gitleaks`, with
      `IRIS_SKIP_HOOKS=1` as the announced bypass

## 3. State the non-goals

- [ ] 3.1 List what Iris does not defend against: an agent running with
...
- [ ] 3.2 Add `IRIS_ALLOW_ANY_PLATFORM=1`, which bypasses the macOS-only admission check
      and puts the app in untested territory
- [ ] 3.3 Add the `~/.claude` isolation as a non-goal too. Pinning `CLAUDE_CONFIG_DIR`
...
- [ ] 3.4 Address prompt injection from **non-audio** sources — file content, tool
...
- [ ] 3.5 Say what *would* be a vulnerability: a path that dispatches work without
...
- [ ] 4.1 Link `SECURITY.md` from README, which currently contains no security section
- [ ] 4.2 Link to the capability specs rather than restating them, so the policy cannot
      drift from the behaviour it describes

## 5. Verify

- [ ] 5.1 Run the five gates. `spec:check` scans only `openspec/specs/` and will pass
      trivially — it proves nothing about this change
- [ ] 5.2 Re-read every claim against the source it cites and confirm the document
...
- [ ] 5.3 Confirm the reporting link works before publishing — task 1.1 must be done, not
      merely planned
- [ ] 5.4 Confirm no real credential, path, or internal address leaked into the document
