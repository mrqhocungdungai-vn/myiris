...
- [x] 3.1 Add the tuning constants to `VaultGalaxy.tsx` beside `ZOOM_MIN_RADIUS`/`ZOOM_MAX_RADIUS`: `LABEL_MAX_DISTANCE = 180`, `LABEL_BUDGET = 24`, `LABEL_WORLD_HEIGHT = 5`, `LABEL_Y_OFFSET = 6`, `SELECT_INTERVAL_MS = 100`, each with the reasoning from design.md D9 — superseded by the manual pass (5.12/design.md D11): `LABEL_MAX_DISTANCE` is now `Infinity` and `LABEL_BUDGET` is now `LABEL_BUDGET_CEILING = 500`, a cap on a pool sized to the vault's own note count rather than a small fixed target
