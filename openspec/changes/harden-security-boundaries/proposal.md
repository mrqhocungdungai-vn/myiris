...
Equally serious, and independent of the voice layer: nothing constrains navigation. There is no `will-navigate` guard, no `setWindowOpenHandler`, and no `dragover`/`drop` handling, so dropping a file onto the window navigates the window that carries the IPC preload — handing `window.iris` to foreign content, which the permission handler then grants microphone and camera to without checking who is asking.
