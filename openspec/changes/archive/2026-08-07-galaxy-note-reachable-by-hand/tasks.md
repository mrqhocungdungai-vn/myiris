...
- [x] 8.7 Manual pass on the sight specifically: whether aiming by hand actually lands, and whether the dolly stalls from the anchor flapping between neighbours mid-zoom
...
- [x] 8.10 Investigate and fix the "now VERY HARD, after all these fixes" regression via a multi-agent workflow (D19): `zoomRadius` is a memoryless ratio law with no per-frame damping (a pre-existing defect, independent of 8.8/8.9) that D17/D18 made bite far more often by reseeding its reference on every retarget; also found and fixed a concrete gate bug (any node graze within `ANCHOR_THRESHOLD_PX` unconditionally cleared the retarget dead-band). Fix: the retarget gate (`PIVOT_RETARGET_DEAD_BAND_PX`, renamed from `POINT_PIVOT_DEAD_BAND_PX`) now covers nodes too, not only points; a new `easeRadius` (mirroring `easeAnchor`) damps the radius law's output instead of applying it directly. Deliberately did NOT adopt the investigation's own proposal to remove mid-zoom retargeting entirely — the user has asked for move-while-zoom to work, across all three reports, not for its removal
...
- [x] 9.8 Manual pass: whether "turn around the mark" reads well when the sight is off to one side (the aim eases onto it, so the view swings by that offset), and whether the search makes the rail worth having
...
- [x] 10.9 Fix the anchor ring covering the note's title (D22), reported from the pass that confirmed 10.1-10.8 work: the rings drew with `renderOrder: 2` + `depthTest: false` against labels at the default 0, so a mark always painted over the name of the note it marked — labels now draw above the marks, and the marks are slimmer and dimmer since they were sized for a camera much further out than D21's arrival distance
- [x] 10.10 Make the lock hold in TIME, not just in space (D23), from the pass that confirmed 10.9: add `zoomLockStep` mirroring `dwellStep` — acquiring is instant, switching costs `ZOOM_LOCK_HOLD_MS`, losing the candidate abandons the charge; remove `PIVOT_RETARGET_DEAD_BAND_PX`, which asked "did the hand travel far enough" and so could never separate a deliberate move from a wobble
- [x] 10.11 Add the acquiring mark — a ring that closes onto the note as the hold charges, shown idle and engaged alike, so "there is a note here and it is about to lock" is distinguishable from empty space and the wait is not dead time
- [x] 10.12 Unit-test `zoomLockStep`: instant first acquisition, no switch before the hold, switch after it, a flapping candidate never switching, a charge abandoned on losing the sight, the lock kept over empty space, and progress reported across the charge
...
- [x] 10.20 Split the two zooms into distinct pose pairs (D26): two open palms zoom the view centre, fist + open palm reels in on the locked note — so which zoom is running is visible in the hands instead of depending on whether something is locked
- [x] 10.21 Extend "a driving pose may not aim" to the whole frame: while ANY hand is a fist, nothing aims — this is what stops a dropped pose mid-zoom leaving one palm in frame, reading as an aim, and re-locking onto a different note
- [x] 10.22 Add `zoomSpan` so both pose pairs share one measurement, and unit-test it alongside `reelsToLock` and the no-aim-while-driving rule
- [x] 10.23 Manual pass on the completed vocabulary: aiming at a note and reeling in arrives at it, the fist turns the view around the locked note, and stepping between notes traverses the sphere. Confirmed working on the seeded vault.

**Note on 6.4 / 8.7 / 9.8:** these were written against camera models that D20-D26 replaced — the fist orbit they describe was removed and later restored on different terms, and the "turn around the mark" sight they test was superseded when aiming moved to a single open palm. Each was run at the time and produced the reports that drove D17-D26; they are checked off as spent rather than as describing behaviour that still exists. The shipped model's own pass is 10.23.
