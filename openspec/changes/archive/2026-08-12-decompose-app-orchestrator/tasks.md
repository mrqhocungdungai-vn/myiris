...
- [x] 1.1 Confirm the file-size ratchet is in the lint gate and `src/App.tsx` is its largest entry (`scripts/file-size-baseline.json`). It is the only automated signal that this change is progressing, and the only thing that stops the reduction silently reversing later.
- [x] 1.2 Confirm `scripts/harness-globs.test.mjs` is in the suite. It is what makes a `.tsx` test file *audible* rather than silently uncollected — relevant because this change is deliberately not adding one (proposal, Open Question), and a later reader must not mistake that for an oversight.
- [x] 1.3 Confirm all five gates are green on a clean tree before touching anything. Record `npm test`'s file/test counts alongside the numbers in `baseline.md`.
- [x] 1.4 Establish the per-commit rule: **one domain per commit**, all five gates green before the next, moved code kept verbatim including comments. Do not squash — per-commit revertibility is the only safety net this change has, since `App.tsx` itself has no test.
- [x] 1.5 Before each extraction, run the ownership check that decided the earlier ones: for every binding in the domain, list where it is read. **If a binding is read outside the domain, extracting it creates props rather than removing them** — that domain is not ready and its consumer boundary must be settled first. This is the rule that correctly separated `ClaudeSection` (14 references, 2 props) from `DeckShell` (67 values, do not extract).
...
- [x] 5.2 The deck branch's distinct-value count has fallen from **67 to 46**, and — more importantly — **14 of those 46 are now cohesive domain objects** (`hud`, `work`, `session`, `review`, `listenOnly`, `workstreams`, `orb`, `ambient`, `claudeQuestion`, `appConfig`, `audio`, …) rather than loose bindings. **32 remain loose**, so a `DeckShell` today would still be a 46-prop component and would still reproduce the `HudShell` anti-pattern. **Closed as a follow-up rather than left open**: the remaining work changes *component contracts*, not the orchestrator, and this change's stated goal was to remove the reason `DeckShell` is impossible — not to extract it.
...
- [x] 5.3 Every capability spec under `openspec/specs/` is still true **with no wording change**. `npm run spec:check` is green, and the only living-spec edit made at archive time is the *additive* `renderer-structure` requirement this change proposed — no existing requirement's wording moved. In particular the pre-existing "`App.tsx` acts solely as the orchestrator" sentence is still true verbatim; the new requirement supplies the bound it lacked rather than replacing it.
- [x] 5.4 `electron/preload.cjs` is unmodified — verified across every commit of this change (`0c72fff`…`38581f4`): none touches the file.
- [ ] 5.5 The `renderer-structure` smoke checklist — wake, submit a task, answer a question by click, change a verb's model, switch/create workstream, choose project folder, open reader and history, dwell-open a card, palm-scroll. **Not run at archive time**, and deliberately not ticked: it needs a live `GEMINI_API_KEY`, a Claude credential and a camera, so it is a human step (`npm start`). It is the one piece of verification the automated gates cannot supply, which is exactly the gap the proposal's Open Question records — `App.tsx` has no test and no render harness. Left open on purpose so a reader is not told it passed.
- [x] 5.6 All five gates green on a clean tree, and the suite **grew**: `npm test` reports **131 files / 2231 tests**, against the 2095 baseline (+136). `build` (tsc src + tsc electron + vite, plus the three-dedupe / types-node / plugin-sync checks), `lint` (0 warnings, 0 errors; `[file-size] OK — 226 files, 3 over the ceiling and none grew`), `scan:secrets` and `spec:check` all clean.
- [x] 5.7 `renderer-structure`'s delta records the bound on what "orchestrator" means and is synced into the living spec. Its recorded figure was corrected 747 → **745** to match `scripts/file-size-baseline.json` (the ratchet banked two further lines after the delta was written; a spec that overstates the ceiling is a ratchet that can silently rise). `docs/ARCHITECTURE.md`'s renderer file map rewritten: `App.tsx` is described as a composition root with its exception and reason instead of "by far the largest file in the repo", and a `src/hooks/*.ts` entry now names the domain hooks, the `src/lib/` pure modules that hold their decisions, and the deliberate absence of a shared store.

## 6. Explicitly out of scope

Ticked at archive time to record that each was **confirmed still absent**, not that work was done.

- [x] 6.1 **No jsdom project, no testing-library, no render harness.** Whether to add one is a real decision with its own cost and belongs in its own change (proposal, Open Question). It is listed here so a reviewer knows the omission is deliberate.
- [x] 6.2 **No `React.memo`/`useCallback` pass.** The renderer had 8 such occurrences when this was written and has 11 now — the three added are incidental to hooks that needed a stable identity, not a memoization pass. Adding memoization is a performance change whose effect must be measured, not assumed. This change makes it *possible* by grouping the values that would otherwise defeat it.
- [x] 6.3 **No shared context or store.** Recorded again here because it is the obvious wrong turn: it would satisfy the line count while making every consumer re-render on every change.
