...
- [x] 9.4 `/opsx:archive` — synced the delta specs into `openspec/specs/` and archived. The sync found **eight capabilities this change falsified that the original deltas did not cover** — `voice-decision-relay` (resolved via `answer_po_question`, a tool that no longer exists), `canvas-claude-mcp` ("**both** Claude paths are wired per run", contradicting this change's own delta), `per-role-model-selection` (role-named env vars and a plain-Claude carve-out), `context-supplement-composer`, `pipeline-availability`, `global-agent-runtime`, `run-execution-queue`, `voice-ui-control`. Those were added as MODIFIED deltas before merging, plus 12 RENAMED for requirement titles that contradicted their own bodies. Also fixed: one requirement filed under MODIFIED that did not exist in main (refiled ADDED), and a **pre-existing living-spec defect** — `voice-decision-relay` carried three duplicated requirements, one with two conflicting bodies, deduped keeping the superseding copy.
