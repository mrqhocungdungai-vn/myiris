...
- [x] 2.1 Trigger the DEV soft-gate (switch to a role whose predecessor handoff is missing): the confirm appears as an in-app modal, NOT a native dialog
- [x] 2.2 While the modal is open, the orb keeps animating and audio keeps playing (event loop not blocked) — the core fix
- [x] 2.3 Confirm → the role switch proceeds; Cancel / Escape / backdrop → the switch is aborted (same as old `!ok`)
...
- [x] 4.1 Dwell over "Remove token" and over "New session": no click fires and the "Hold · opening" indicator does not engage; a mouse click on each still works
- [x] 4.2 Dwell over a normal target (task card, PO answer option, step-timeline toggle, close button) still fires exactly as before
- [x] 4.3 Dwell over the gate-confirm modal's Confirm button still works (reversible action stays dwell-operable)
