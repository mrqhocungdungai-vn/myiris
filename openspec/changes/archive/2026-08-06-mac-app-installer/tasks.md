...
- [x] 1.1 Make `package:mac:host` actually build the host architecture — pass an
...
- [x] 1.2 Verify on a clean checkout: `npm ci` installs only the host-matching Claude
...
- [x] 1.3 Correct CLAUDE.md's command table: `dist:mac` is identical to `package:mac`
...
- [x] 1.4 Decide whether `dist:mac` should keep existing as a duplicate of `package:mac`
      or gain a real distributable target. Out of scope to implement; record the decision

## 2. The install script

- [x] 2.1 Add `scripts/install-mac.mjs` and an `npm run install:mac` entry that builds,
      packages for the host arch, installs, and launches
- [x] 2.2 Select the source bundle by **architecture**, not first match. electron-builder
...
- [x] 2.3 Verify the selected bundle's executable matches `process.arch` (`lipo -archs`
      or `file`) and refuse otherwise
- [x] 2.4 Quit a running Iris with `osascript -e 'tell application "Iris" to quit'` —
...
- [x] 2.5 Read the shutdown budget from the same source the app uses
...
- [x] 2.6 Distinguish "nothing was running" from "the quit request was refused".
      `osascript` needs Automation permission; denied, it fails silently and the only
      symptom is the wait timing out
- [x] 2.7 Scope the running-instance probe to the installed path
...
- [x] 2.8 Copy with `ditto`, which preserves the bundle's symlink farm and permissions
- [x] 2.9 Clear quarantine with `xattr -dr com.apple.quarantine`, **not** `xattr -cr`.
      Stripping all attributes would destroy `com.apple.cs.*` signature xattrs on any
      bundle that is ever signed
- [x] 2.10 Delete `ELECTRON_RUN_AS_NODE` from the environment before launching — a shell
      exporting it starts Iris headless with no window. `scripts/run-electron.mjs`
      already does this; the same footgun applies here
- [x] 2.11 Launch the installed app at the end

## 3. Guard the destructive step

- [x] 3.1 The destination is a **hard-coded constant**. Do not accept it from an argument
...
- [x] 3.2 Before removing anything, verify the target is Iris's own bundle: a directory
...
- [x] 3.3 Refuse and exit with a clear message if the target is not what is expected,
      rather than removing it anyway
- [x] 3.4 Fail loudly on any step that errors rather than continuing and leaving a
      half-replaced bundle
- [x] 3.5 Extract target resolution and the identifier/arch verification into a pure,
...
- [x] 4.1 `release/mac*/Iris.app` are the build products, not a staging directory.
...
- [x] 5.1 Document `npm run install:mac` in README next to the existing build/run commands
- [x] 5.2 Tell the user the installed app reads `~/.iris/.env`, not the repo's `.env`, and
      what to put there — otherwise the first launch fails on a missing `GEMINI_API_KEY`
- [x] 5.3 State that the app is unsigned and what the script does about quarantine
- [x] 5.4 Fix README's "right-click and choose Open" line — that path was removed in
      macOS Sequoia

## 6. Verify

- [x] 6.1 Run the five gates. Note `spec:check` only scans `openspec/specs/`, so it is a
      no-op for this change and proves nothing about it
- [x] 6.2 Confirm the new script is oxlint-clean — `scripts/` is in the lint targets at
      zero warnings
- [x] 6.3 Follow the conventions in `scripts/`: a bracketed log tag on every line, a
