...
- [x] 1.1 Add `src/lib/boot-gate.ts` exporting a pure function that takes the
...
- [x] 1.2 Encode the four transitions in it: rising edge of `running` while not
...
- [x] 1.3 Add `src/lib/boot-gate.test.ts` with one case per scenario in the delta
...
- [x] 1.4 Assert in 1.3 that `reportBootDone` is true exactly once across a cold
      start, and never across the reconnect or shutdown sequences

## 2. Wire it into the renderer

- [x] 2.1 Replace the derived `booting` expression at `src/App.tsx:828` with state
      driven by `boot-gate`, fed from a ref holding the previous `sidecarRunning`
- [x] 2.2 Drive `<BootSequence visible={...}>` (`src/App.tsx:1856`) from that state
- [x] 2.3 Fire `window.iris.notifyBootDone()` from the gate's `reportBootDone`
      rather than from the falling edge of `booting` (`src/App.tsx:829-835`), and
      remove `prevBootingRef` once nothing reads it
- [x] 2.4 Check whether any other consumer reads `booting`; re-point or leave it
