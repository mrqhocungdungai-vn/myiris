...
2. **Token subscription cho PO** (PO là một phiên Agent SDK stateful, không kế thừa đăng nhập `claude` tương tác của bạn; DEV thì không cần cái này):
   ...
   claude setup-token
   ```
   Dán kết quả vào `.env` với tên `CLAUDE_CODE_OAUTH_TOKEN` (xem `.env.example`).
...
| PO báo lỗi thiếu token | Chưa có `CLAUDE_CODE_OAUTH_TOKEN` | Chạy `claude setup-token`, dán kết quả vào `.env`, khởi động lại Iris |
