...
## Voice decision relay (stateful verbs only)

- A stateful run may call `AskUserQuestion` mid-turn (its persona and its system prompt say so explicitly — the opposite of the stateless clause's "never ask"). The SDK's `canUseTool` callback in `po-session.mjs` intercepts it and awaits an answer from `askUserQuestionViaVoice` in `run-stream.mjs`.
- **The asymmetry is enforced, not just stated.** Measured: `AskUserQuestion` is only offered to the model when a `canUseTool` callback is present, and `disallowedTools` removes it even then. Every stateless verb carries `disallowedTools: ["AskUserQuestion"]` from the registry **and** a `canUseTool` that aborts the run with a diagnostic if it is ever reached, so a headless run can never wait for an answer nobody is listening for. `investigate` additionally carries `Write`/`Edit`/`NotebookEdit` in that list — investigating does not modify, and that has to be structural too; a withheld edit tool is denied *without* ending the run, since the model can still answer. The prompt is the explanation; the configuration is the guarantee.
...
- Tool-use permission mode stays `bypassPermissions` — only `AskUserQuestion` pauses; every other tool call auto-allows. The SDK warns that `bypassPermissions` shadows `canUseTool`; measured, it fires for `AskUserQuestion` anyway, which is the asymmetry the stateful path depends on.
