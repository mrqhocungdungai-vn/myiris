...
|---|---|---|---|---|---|
| A | PO turn không settle → kẹt slot vĩnh viễn | 🔴 Critical | `po-session.mjs` | ~5 dòng | [ ] |
| A' | `announceClaudeCompletion` không phân biệt status → đọc to lỗi cho run người dùng tự hủy | 🟠 Medium | `main.mjs`, `run-queue.mjs` | ~6 dòng | [ ] |
...
| I | Không có watchdog / PO không hủy được / subprocess mồ côi | 🟠 Design | `run-queue.mjs`, `main.mjs` | lớn | [ ] |
...
### BUG E — Gemini được báo "started" cho run đã fail đồng bộ 🟡
...
#### I.1 — Watchdog theo run ⟵ **ĐƯA LÊN ĐỢT 0, LÀM TRƯỚC BUG A**
