...
### BUG K — Spec và code mâu thuẫn về hủy run đang queued 🟡 — ✅ **XONG** (`reconcile-queued-cancel`, 2026-07-22)
...
**Đã xử lý (`reconcile-queued-cancel`, 2026-07-22):** đi theo đúng hướng đã đề xuất — sửa **spec**, không sửa code path. `run-queue.mjs`'s `stop()` queued branch giờ set `run.finalized = true` ngay tại chỗ (một dòng), **không** gọi `finalize()` — vì `dequeueNext()` bên trong `finalize()` sẽ unconditionally clobber slot của run đang active. Comment cũ ("no announcement to make") được thay bằng lý do thật. Kết quả: bất biến "`finalized` ⇔ terminal" giờ đúng cho **mọi** run, once-guard bảo vệ được cả run bị hủy lúc còn queued, và spec `run-execution-queue`'s "Stopping a run" được viết lại để mô tả đúng hành vi này (silent, no-dequeue) thay vì "finalize... immediately" sai lệch. Không đổi hành vi quan sát được nào. Đóng nốt hạng mục cuối của Wave 0.
...
### BUG D — Card hiển thị activity log như thể là kết quả của Claude 🟡 — ✅ **XONG**
...
### BUG F — `useHandControl` setState mỗi frame → re-render toàn bộ cây 🟠 — ✅ **XONG** (`bound-hand-and-orb-render-cost`, 2026-07-22)
...
### BUG G — Orb trong Glass HUD không bao giờ dừng render 🟠 — ✅ **XONG** (`bound-hand-and-orb-render-cost`, 2026-07-22)
...
|---|---|---|
| `ReactorCore` cấp phát trong hot loop — ✅ **XONG** (`unstall-render-and-audio`, 2026-07-22) | `ReactorCore.tsx:25-28, 66, 133, 139-181` | `new THREE.Color` / `new THREE.Vector3` mỗi frame ≈ 700 alloc/giây → GC ngắt lịch phát audio. Palette là tĩnh (17-23) → hoist ra module scope, dùng `.copy()`/`.set()`. **Đã vá:** `PALETTE_COLORS` (module-scope, parse-once) + scratch `_scaleVec`; `useFrame` (kể cả `Ripples`) giờ chỉ `.copy()`/`.set()` vào material/transform có sẵn — không allocation nào trên hot path, màu/chuyển động không đổi |
| `ScriptProcessorNode` trên main thread — ✅ **XONG** (`unstall-render-and-audio`, 2026-07-22) | `useAudioPipeline.ts:102-121` | Deprecated; `onaudioprocess` chạy **trên main thread**, downsample + IPC mỗi ~21ms ở 48kHz, tranh chấp trực tiếp với React và three.js. Đích đến đúng là `AudioWorklet`. Đây là nguồn gốc của tiếng audio giật khi bug F đang hoành hành. **Đã vá:** downsample chuyển sang `src/lib/downsample.ts` (thuần, unit-tested — `downsample.test.ts`); `ScriptProcessorNode` thay bằng `AudioWorkletNode` (`src/worklets/mic-downsample.js`, cùng thuật toán, chạy trên audio rendering thread); gửi 16kHz PCM byte-identical, tap `analyser` cho HUD giữ nguyên |
| `window.confirm` khóa cả event loop — ✅ **XONG** (`harden-interaction-safety`, 2026-07-22) | `App.tsx:495` | Modal native trong renderer Electron **chặn toàn bộ main thread**: rAF dừng, orb đóng băng, lịch phát audio khựng, MediaPipe dừng. Kết hợp dwell-click (810, khớp `button, a, [data-task-id], [role="button"]`) → tay lơ lửng trên chip role có thể đóng băng audio. **Đã vá:** thay bằng modal in-app non-blocking — state `confirm: { message, resolve } \| null` + helper `askConfirm(message): Promise<boolean>` (`ConfirmModal.tsx` mới), gate handler `await askConfirm(...)` thay `window.confirm(...)`, semantics confirm/cancel giữ nguyên. Spec delta: `main-thread-budget` — một requirement **ADDED** (không dùng dialog đồng bộ chặn main thread) |
| Dwell-click có thể bấm nút phá hủy — ✅ **XONG** (`harden-interaction-safety`, 2026-07-22) | `App.tsx:791` | Không có cơ chế opt-out. `SetupPanel` có nút Remove token / Save. Cần thuộc tính `data-no-dwell` cho các control nguy hiểm. **Đã vá:** vòng dwell coi `actionable.closest('[data-no-dwell]')` khác null là "không có mục tiêu" (không `.click()`, không hiện chỉ báo "Hold · opening"); gắn `data-no-dwell` lên nút Remove token (`SetupPanel.tsx`), nút New session (`SessionSwitcher.tsx`), và nút chuyển project folder (`ProjectBar.tsx`) — chuột và giọng nói vẫn dùng được các nút này bình thường. Nút Confirm/Cancel của gate confirm **không** bị gắn (hành động khả nghịch, vẫn dwell được). Spec delta: `two-hand-gestures` — một requirement **MODIFIED** (dwell loại trừ `[data-no-dwell]`) |
| Bật webcam ngầm khi wake — ✅ **XONG** (`fix-gesture-camera-lifecycle`, 2026-07-22) | `App.tsx:750` (cũ) | `handControl = true` vô điều kiện → đèn camera sáng mỗi lần thức, tải MediaPipe WASM + model từ CDN (`useHandControl.ts:35-37`), chạy GPU inference — dù người dùng không cần gesture. **Đã vá:** `iris.handControlEnabled` persisted (mặc định **off**), `start()` không còn ép `true`; toggle đổi thành `toggleHand()` ghi preference như `toggleSounds` |
| Lỗi camera dính vĩnh viễn — ✅ **XONG** (`fix-gesture-camera-lifecycle`, 2026-07-22) | `useHandControl.ts:88` (cũ) | `error` không bao giờ được clear khi retry thành công → một lần `getUserMedia` lỗi thoáng qua là `handError` kẹt suốt đời tiến trình. **Đã vá:** `setError(null)` khi acquire thành công (`setup()`, sau `getUserMedia`/`video.play()`) và khi `!enabled`; đường lỗi (`catch`) và "không silent retry/fallback" giữ nguyên |
| Chunk audio rò rỉ sau khi ngắt lời — ✅ **XONG** (`fix-audio-playback-lifecycle`, 2026-07-22) | `useAudioPipeline.ts:160-196` | `playGeminiAudio` await `context.resume()` (168) rồi mới tính `startAt` từ `playbackTimeRef` (192). Nếu `flushPlayback` (146-158) chạy trong lúc await — đúng cái mà barge-in làm (`App.tsx:251`) — chunk đang bay được lên lịch theo timeline **trước flush** và vẫn phát. Triệu chứng: Iris nói nốt một mẩu sau khi đã bị cắt lời. **Đã vá:** `flushEpochRef` — `flushPlayback` tăng epoch, `playGeminiAudio` chụp epoch lúc vào hàm và bail (không `source.start`, không tiến `playbackTimeRef`) nếu epoch đổi trước lúc `source.start`; phủ mọi `await` kể cả `context.resume()`. Spec mới: `audio-playback` |
| Output `AudioContext` không bao giờ đóng — ✅ **XONG** (`fix-audio-playback-lifecycle`, 2026-07-22) | `useAudioPipeline.ts:210-215` | `stop()` đóng input context qua `stopCapture` (137) nhưng để `outputContextRef.current` mở vĩnh viễn. **Đã vá:** `stop()` sau `flushPlayback()` đóng và null `outputContextRef`/`outputAnalyserRef`, reset `playbackTimeRef`; `playGeminiAudio`'s `?? new AudioContext()` và rebuild analyser khi context lệch đã tự tái tạo ở lần phát kế tiếp, không cần code thêm. Spec mới: `audio-playback` |
| `handleSidecarEvent` stale closure — ✅ **XONG** (`harden-slot-release-and-event-subscription`, 2026-07-22) | `App.tsx:245, 575-739` | Đăng ký với deps `[hasBridge]` → giữ closure của render 0 vĩnh viễn. Sống sót vì mọi nhánh chỉ dùng setter và functional updater (React đảm bảo ổn định). Chỉ cần một người đọc `pendingPoQuestion` hay `sortedTasks` bên trong là **im lặng đọc giá trị render 0 mãi mãi**. **Đã vá:** `sidecarHandlerRef` (`useRef`, refresh mỗi render qua một `useEffect` không-dependency) — đăng ký `[hasBridge]` giữ nguyên (không churn native listener), nhưng dispatch qua `sidecarHandlerRef.current(event)` nên luôn gọi đúng closure mới nhất. Comment tại chỗ đăng ký giải thích lý do |
...
- `cancelActive()` để PO turn hủy được thật — **mâu thuẫn trực tiếp** với scenario hiện có (`run-execution-queue`: *"Stopping an active PO turn → the call returns the run's current status unchanged and the turn continues to completion"*). Cần MODIFIED requirement, không phải bổ sung. **✅ XONG** (`make-po-turns-cancellable`, 2026-07-22).
- `detached: true` + kill cả process group. **✅ XONG** (`bound-shutdown-teardown`, 2026-07-22, xem I.3 bên dưới).
- `before-quit`: `preventDefault()` + await teardown, có deadline cứng. **Phần này đã là drift, không phải gap** — `po-live-session` đã yêu cầu *"Live session ends cleanly on app shutdown … without leaving an orphaned Claude process."* **✅ XONG** (`bound-shutdown-teardown`, 2026-07-22, xem I.4 bên dưới).
- `finalize` idempotent với run không-active (xem BUG K) — **✅ XONG** (`harden-slot-release-and-event-subscription`, 2026-07-22). Note ban đầu ("đã thoả mãn sẵn bởi once-guard, không cần code mới") được tinh chỉnh lại: an toàn *by convention* hôm nay (mọi caller đều finalize run active), nhưng once-guard không ngăn `finalize()` trên một run không giữ slot khỏi xoá `active`/timer của run đang chạy thật. Đã thêm guard cấu trúc `if (active === runId)` quanh `clearIdleTimer()`/`idleSuspended`/`dequeueNext()` — giờ an toàn *by construction*, không phụ thuộc kỷ luật gọi hàm.

#### I.3 — `detached: true` + group-aware `killChild` — ✅ **XONG** (`bound-shutdown-teardown`, 2026-07-22)

DEV `spawn` (`main.mjs`) nhận thêm `detached: true` (không `unref()` — parent vẫn quản lý child) nên trở thành process-group leader trên POSIX. `run-queue.mjs`'s `createRunQueue` nhận thêm dependency injected `killChild(child, signal)` (song song `startRun`/`cancelRun`, mặc định `(child, signal) => child.kill(signal)` giữ đúng hành vi cũ nếu ai đó dùng queue mà không inject) — `killWithEscalation` gọi `killChild` thay vì `run.child.kill` trực tiếp, không đổi gì khác ở luồng escalation/finalize. `main.mjs` cung cấp implementation cross-platform: POSIX `process.kill(-child.pid, signal)` (bọc try/catch, fallback `child.kill(signal)` nếu group đã mất hoặc thiếu `pid`); Windows `spawn("taskkill", ["/pid", pid, "/T", "/F"])` (`signal` chỉ mang tính tham khảo). Kết quả: dừng một run giờ diệt luôn cả cây subprocess con (bash, MCP server dưới `bypassPermissions`) — không còn mồ côi. `run-execution-queue` spec — MODIFIED requirement "Stopping a run": escalation clause giờ mô tả group-kill qua injected hook.

#### I.4 — `before-quit` bị chặn (bounded), có await, chạy đúng một lần — ✅ **XONG** (`bound-shutdown-teardown`, 2026-07-22)

`po-session.mjs`'s `closePoSession`/`closeAllPoSessions` giờ trả về promise teardown (`query.return()`), caller fire-and-forget cũ (reset, đổi workstream) bỏ qua giá trị trả về, không đổi hành vi. `main.mjs`'s `before-quit` viết lại hoàn toàn: guard module-level `shuttingDown` (bỏ qua tín hiệu quit lặp lại), `event.preventDefault()`, rồi `Promise.race([shutdownTeardown(), deadline]).finally(() => app.exit(0))` — `shutdownTeardown()` await `stopLive()`, group-kill mọi DEV child còn sống qua `killChild`, rồi `await closeAllPoSessions()`; `deadline` là timer `IRIS_SHUTDOWN_DEADLINE_MS` (mặc định 8000ms, tài liệu trong `.env.example`) `unref()`'d. `will-quit`'s `globalShortcut.unregisterAll()` không đổi. Capability mới: `app-shutdown` (2 requirement ADDED — quit chặn tới khi teardown xong hoặc deadline hết; teardown chạy tối đa một lần). Đóng nốt drift với `po-live-session`'s "Live session ends cleanly on app shutdown" (không cần sửa spec đó, code giờ đã tuân theo).
...
| **J** | **Lệch khỏi phán quyết ban đầu — hoá ra là Gap** | Bảng này từng xếp J vào "không đụng spec". Khi làm `deny-pending-question-on-reset` mới lộ ra: `voice-decision-relay` chỉ nói callback phải "được settle" và turn "bị teardown" khi reset — im lặng về việc settle bằng `allow` hay `deny`. Đã thêm một requirement **tên riêng biệt** (không đụng vào hai bản đã trùng lặp sẵn có trong spec đó) để phát biểu rõ: reset phải `deny`, không được trả lời bằng câu mặc định giả |
| **C, D, F, G, H** | không đụng spec | |
...
| 2026-07-22 | **BUG J xong** — `deny-pending-question-on-reset`, một commit | `PendingQuestion` (`main.mjs`) giờ settle bằng một **descriptor** thay vì bare answers map: `answer`/`expire` → `{behavior: "allow", answers}` (không đổi hiệu ứng, chỉ đổi vỏ bọc), `abandon` → `{behavior: "deny", message}` với status mới `"abandoned"` (trước đây dùng chung `"timed_out"` của timeout — sai, vì reset không phải timeout). `canUseTool` (`po-session.mjs`) dịch descriptor: `deny` → trả `{behavior: "deny", message}` cho SDK, `allow` → giữ nguyên hành vi cũ. `po-session.mjs` vẫn không biết "reset" khác "timeout" ở đâu — chỉ dịch allow/deny, giữ đúng ranh giới hai module. Ba điểm teardown (`main.mjs:485-486, 504-505, 523-524`) không đổi — `abandon` deny là toàn bộ fix, không cần sửa call site. Banner PO-question ở cả deck và HUD đã tự dismiss đúng vì logic hiện tại clear trên **mọi** status khác `"pending"`, không riêng `"timed_out"`/`"answered"` — không cần sửa `App.tsx`. Spec delta: `voice-decision-relay/spec.md` — một requirement **tên riêng mới** ("A session reset denies a pending question rather than answering it"), không đụng vào hai cặp requirement/scenario đã trùng lặp sẵn có trong spec đó (dedupe là việc khác, không phải bug này). `npm test` 28/28 pass, `npm run build` sạch, `openspec validate` pass. Tasks 5.3-5.5 (manual: PO hỏi rồi bị reset giữa chừng → deny, turn `CANCELLED`, không ghi file vào cwd cũ; timeout thật vẫn áp default; trả lời bằng giọng nói và bằng UI click vẫn resume turn bình thường) **chưa verify bằng tay trên app thật — cần người dùng chạy qua trước khi archive**. 17/20 tasks xong (3 manual còn lại) |
| 2026-07-22 | **BUG D xong** — `show-real-result-not-activity-log`, một commit | Thêm hàm thuần `resolveMergedString(raw, existing)` (`src/lib/tasks.ts`, bên cạnh `readString`): lấy giá trị sự kiện bất cứ khi nào sự kiện **có mang** một chuỗi (kể cả `""`), ngược lại giữ giá trị cũ — phân biệt "rỗng" với "không có" mà `readString` một mình không làm được. `App.tsx:668-669` (`output`/`error`) đổi từ `X || existing?.X` sang `resolveMergedString(event.X, existing?.X)`; hai biến cục bộ `output`/`error` (`App.tsx:633-634`) bị xóa vì không còn dùng. `error` cũng được vá cùng lúc — cùng một lỗi tiềm ẩn `|| ` y hệt. Hạ tầng Vitest mở rộng ra `src/**/*.test.ts` (env `node`, chỉ helper thuần, chưa đụng DOM/React) và `tsconfig.json` thêm `"exclude": ["src/**/*.test.ts"]` để `tsc --noEmit` không quét file test — Vite vẫn không đóng gói file test chưa import vào `dist/`. Spec delta: `task-step-timeline` — một requirement **tên riêng mới** ("A completed card shows the run's result, never the activity log") với 3 scenario (rỗng → không hiện activity log, không rỗng → hiện đúng kết quả, update không mang trường result → giữ nguyên text đang hiện) — lệch khỏi quy ước drift-vs-gap "BUG D không có spec", chủ đích vì reducer này sắp bị viết lại ở Đợt 3 (sidecar-event reducer refactor) nên cần chốt hành vi bằng spec trước khi refactor tới. `npm test` 32/32 pass (4 test mới cho `resolveMergedString`), `npm run build` sạch, `openspec validate` pass. Tasks 5.3-5.4 (manual: chạy task Claude trả kết quả rỗng → card hiện rỗng/placeholder chứ không phải activity log; chạy task kết quả không rỗng → không đổi hành vi) **đã được người dùng verify bằng tay trên app thật, đạt cả hai**. 19/19 tasks xong, sẵn sàng archive |
| 2026-07-22 | **BUG K xong** — `reconcile-queued-cancel`, một commit | Sửa spec, không sửa code path (code đã đúng từ đầu). `run-queue.mjs`'s `stop()` nhánh `QUEUED` thêm một dòng `run.finalized = true` ngay tại chỗ, **không** gọi `finalize()` — vì `dequeueNext()` bên trong sẽ unconditionally clobber slot của run đang active (run bị hủy lúc còn queued chưa từng giữ slot). Comment cũ ("no announcement to make") thay bằng lý do thật: không gọi `finalize()` vì `dequeueNext()` sẽ giải phóng nhầm slot của run active; run được đánh dấu finalized trực tiếp để once-guard bảo vệ nó. Kết quả: bất biến "`finalized` ⇔ terminal" giờ đúng cho **mọi** run — đóng nốt hạng mục treo cuối cùng của Wave 0. Spec delta: `run-execution-queue/spec.md` — requirement "Stopping a run" viết lại từ "finalize... immediately" (sai, nguy hiểm nếu đọc theo nghĩa đen) sang mô tả đúng hành vi (silent, no-dequeue, marked finalized); thêm 2 scenario mới ("Cancelling a queued run does not disturb the active run", "A cancelled queued run cannot be re-finalized"). Test: sửa test BUG K cũ (`finalized` lật từ `not.toBe(true)` sang `toBe(true)`) + 2 test mới trong `run-queue.test.mjs` (tổng 34 test, tất cả pass). `npm test` 34/34 pass, `npm run build` sạch, `openspec validate` pass. Không đổi hành vi quan sát được nào — chỉ đóng một lỗ hở tiềm ẩn (chưa từng bị khai thác) và gỡ mâu thuẫn spec↔code. Tasks 3.3-3.4 (manual: submit task khi đang có run chạy → hủy task đang xếp hàng, card hiện `cancelled`, Iris im lặng, run active vẫn tiếp tục bình thường; xác nhận không hồi quy đường stop run active/PO) **đã được người dùng verify bằng tay trên app thật, đạt cả hai**. **15/15 tasks xong.** **Đóng Wave 0.** |
| 2026-07-22 | **BUG F + G xong** — `bound-hand-and-orb-render-cost`, hai commit — **mở đầu Wave 1** | **BUG F:** `useHandControl.ts` giữ dữ liệu per-frame (`point`, `landmarks`, `pinchDistance`) trong một `stateRef` cập nhật mỗi rAF; `state` React chỉ `setState` khi phần **ngữ nghĩa** đổi (`present`, `gesture`, `pointing`, `openPalm`, `fist`, số lượng/id bàn tay) — so sánh bằng hàm thuần `semanticEquals` mới (`src/lib/hand.ts`, có test). Nhánh không-có-tay giờ chỉ publish một lần lúc **chuyển trạng thái** vào rỗng (cờ `hadHand`), không phải mỗi frame. `App.tsx`: bộ đếm dwell-click chuyển từ effect phụ thuộc `hand.point?.x/y` (chính là nguồn ép re-render 60fps cũ) sang vòng rAF riêng đọc thẳng `stateRef`, chỉ publish state `dwellActive`/`dwellFired` khi vào/ra mục tiêu hoặc bắn click — `handAction` memo bỏ `hand.point?.x/y` khỏi deps. **Vượt phạm vi liệt kê trong Impact của đề xuất:** áp dụng luôn kiểu đọc-ref-imperative này cho `HandReticles`, `HandSkeleton` (dùng chung bởi `CameraDock` và `HudShell`'s camera HUD), và vòng lặp two-palm-resize/scroll của `ReaderOverlay` — nếu không, ba chỗ này sẽ đứng hình giữa các lần chuyển ngữ nghĩa (điểm/landmark không còn nằm trong `state` publish mỗi frame nữa), hồi quy scenario "camera dock shows both skeletons" và "two-palm reader resize" của `two-hand-gestures` mà Goals của design.md yêu cầu giữ nguyên. **BUG G:** `HudShell`'s `<ReactorCore>` nhận `running={sidecarRunning}` (thức, không theo focus — HUD là overlay ambient luôn hiện) cùng `orbRotationRef`/`orbScaleRef` đã có sẵn từ vòng fist-rotate/pinch — orb HUD giờ dừng render khi ngủ và phản hồi đúng fist-rotate/pinch-scale; đường deck (`CenterStage`, `orbRunning = sidecarRunning && windowFocused`) không đổi. Spec delta: `orb-expressions` — một MODIFIED requirement làm rõ HUD (dừng khi ngủ, chạy tiếp khi thức dù mất focus) khác deck (dừng cả khi ngủ lẫn mất focus) một cách **chủ đích**, chặn trước việc một thay đổi sau này "sửa" orb HUD dừng theo blur rồi phá luôn overlay glanceable. Test mới: `src/lib/hand.test.ts` (5 test cho `semanticEquals`, tổng `npm test` 39/39 pass), `npm run build` sạch, `openspec validate --strict` pass. Tasks 3.1-3.4 (không có tay trong khung hình → App không re-render ~60fps; reticle bám tay mượt; dwell-click "Hold · opening" hiện trong ~300ms, bắn đúng một lần, rời-rồi-vào-lại tái vũ trang, cả deck lẫn HUD; fist-rotate/pinch-scale/two-palm-resize/open-palm-scroll đều hoạt động) và 5.1-5.3 (HUD ngủ → orb dừng render, thức lại → resume; HUD thức nhưng mất focus → orb vẫn chạy tiếp; HUD fist-rotate xoay orb, pinch scale orb) **đã được người dùng verify bằng tay trên app thật, đạt cả bảy**. **26/26 tasks xong.** Kế tiếp: BUG H (`pushActivity` ~17KB/dòng) vẫn còn treo trong Wave 1 |
| 2026-07-22 | **BUG H xong** — `coalesce-activity-updates`, một commit | `pushActivity`'s emit ra renderer giờ đi qua một trailing-throttle nguồn: `electron/coalesce.mjs` mới, `createTrailingThrottle(fn, ms)` thuần trả `{schedule, cancel}` — `schedule` gom nhiều lệnh gọi trong một interval về đúng một lần gọi `fn` cuối cùng (trailing edge, args mới nhất), `cancel()` huỷ timer đang chờ mà không gọi `fn`. `main.mjs`: `pushActivity` vẫn `run.activity.push(...)`, cap 80 dòng, và `runQueue.heartbeat()` per-line y hệt trước (D2) — chỉ riêng `emitEvent(...)` đổi từ gọi trực tiếp sang `activityThrottle.schedule(run)` qua một throttle module-level duy nhất (slot thực thi singleton ⇒ tối đa một run có throttle sống), interval `IRIS_ACTIVITY_EMIT_INTERVAL_MS` mặc định 150ms (mới, tài liệu trong `.env.example`). `tool_start`/`tool_end` (step timeline) không đụng tới, vẫn realtime. `onFinalized` giờ gọi `activityThrottle.cancel()` ngay đầu callback (trước khi kiểm tra `started_at`) để một emit activity đang chờ không thể bắn sau update kết quả thật của `finalize()` — chặn hồi quy với "A completed card shows the run's result, never the activity log" (BUG D). Spec delta: `task-step-timeline` — một requirement **ADDED** mới ("Activity emission is rate-bounded without starving liveness or clobbering results") chốt ba bất biến: step timeline không bị ảnh hưởng, kết quả cuối không bị đè, tín hiệu tiến độ cho watchdog idle không bị chậm lại. Test mới: `electron/coalesce.test.mjs` (3 test Vitest fake-timer: burst dồn về 1 lần gọi với args cuối, nhiều interval liên tiếp gọi đúng số lần, `cancel()` trước trailing edge chặn hẳn và một `schedule` sau đó vẫn hoạt động), `npm test` 42/42 pass, `npm run build` sạch, `openspec validate` pass. Tasks 6.2-6.5 (manual: đo tốc độ re-render bằng React DevTools Profiler trên một DEV run "nói nhiều" thật; xác nhận activity log vẫn cuộn sống trong khi step timeline vẫn realtime; card hoàn tất hiện đúng kết quả thật kể cả ngay sau một burst activity — ca hồi quy D3; một run "nói nhiều" liên tục không bị watchdog idle giết) **đã được người dùng verify bằng tay trên app thật, đạt cả bốn**. **21/21 tasks xong.** Đóng nốt các hạng mục render-path của Wave 1 (F, G, H đều xong); bảng "mục nhỏ khác" (`ReactorCore` allocations, `AudioWorklet`, `window.confirm`) vẫn còn treo |
| 2026-07-22 | **`ReactorCore` alloc + `ScriptProcessorNode` xong** — `unstall-render-and-audio`, hai commit | **Item 1:** `PALETTE_COLORS` (module-scope, parse-once từ `PALETTES` string) + scratch `_scaleVec` (`ReactorCore.tsx`); `useFrame` (cả `ArcReactorScene` lẫn `Ripples`) chuyển từ `mat.color = rgbToColor(...)` (≈8 alloc/frame) và `new THREE.Vector3(...)` sang `.copy()`/`.set()` in-place — không allocation nào trên hot path, màu/glow/chuyển động orb không đổi. `rgbToColor` giờ chỉ chạy một lần lúc build `PALETTE_COLORS`. **Item 2:** `downsampleTo16k` chuyển sang `src/lib/downsample.ts` (thuần, `downsample.test.ts` mới — 3 test: averaging 3:1, identity-rate passthrough, edge case input ngắn hơn một output sample). `ScriptProcessorNode`/`onaudioprocess` (chạy trên main thread) thay bằng `AudioWorkletNode` (`src/worklets/mic-downsample.js`, thuật toán inline **line-for-line giống** bản lib vì AudioWorklet chạy global scope cô lập, không `import` được) — downsample giờ chạy trên audio rendering thread, main thread chỉ nhận PCM 16kHz đã downsample xong qua `port.onmessage` rồi forward `sendAudioChunk` (byte-identical với trước). Tap `analyser` cho HUD level-meter giữ nguyên. `addModule` thử URL Vite-bundled (`new URL(..., import.meta.url)` — Vite inline thành `data:` URL vì file nhỏ hơn ngưỡng 4KB) trước, fallback sang bản copy ở `public/worklets/` (cùng pattern đã dùng cho wakeword `.onnx` models) nếu file:// đóng gói không resolve được; lỗi cả hai đường thì log rõ và dừng capture thay vì âm thầm treo. Spec mới: `main-thread-budget` (2 requirement ADDED — orb loop không alloc per-frame, mic capture/downsample không chạy trên main thread). Test: `npm test` 45/45 pass (thêm `downsample.test.ts`), `npm run build` sạch, `openspec validate` pass. Tasks 2.1-2.3 và 5.1-5.3 (manual: orb giống hệt ở mọi state + allocation probe DevTools; fist-rotate/pinch-scale vẫn hoạt động; mic round-trip trong `npm run dev` và trong bản đóng gói `npm run package:mac` — đây là rủi ro đóng gói lớn nhất của D4; barge-in vẫn ngắt lời đúng) **chưa verify bằng tay trên app thật — cần người dùng chạy qua, đặc biệt bản đóng gói, trước khi archive**. 19/23 tasks xong (4 manual còn lại). Bảng "mục nhỏ khác" còn treo: `window.confirm`→async, dwell-destructive guard/`data-no-dwell`, webcam opt-in, camera-error-stuck, barge-in flush race, output-context leak, stale-closure comment — vẫn mở trước Wave 2 (BUG I.2–I.5) |
| 2026-07-22 | **`window.confirm` + dwell-destructive xong** — `harden-interaction-safety`, hai commit | **Item 3:** state `confirm: { message, resolve } \| null` + helper `askConfirm(message): Promise<boolean>` (`App.tsx`) và `ConfirmModal.tsx` mới (overlay + CSS trong `overlays.css`, tái dùng pattern `*-backdrop`/`*-card` sẵn có) thay cho `window.confirm(...)` ở gate xác nhận DEV (`App.tsx:503`); handler đã `async` sẵn nên chỉ đổi thành `await askConfirm(...)`, semantics confirm/cancel giữ nguyên byte-for-byte, chỉ bỏ chặn main thread. Escape và click backdrop resolve `false` giống Cancel. Xác nhận không còn `window.confirm`/`alert`/`prompt` nào trong `src/`. **Item 4:** vòng dwell (`App.tsx:~813`) coi `actionable.closest('[data-no-dwell]')` khác null là "không có mục tiêu" — không `.click()`, không hiện chỉ báo dwell; gắn `data-no-dwell` lên nút Remove token (`SetupPanel.tsx:333`), nút New session (`SessionSwitcher.tsx`'s `.session-new`), và nút chuyển project folder (`ProjectBar.tsx`) — ba control có mất dữ liệu/không thể hoàn tác. Nút Confirm/Cancel của gate confirm (Item 3) **không** bị gắn — hành động khả nghịch, vẫn dwell được (design D3). Spec delta: `main-thread-budget` — một requirement **ADDED** (không dùng dialog đồng bộ chặn main thread); `two-hand-gestures` — requirement "Universal point-and-hold click" **MODIFIED** (loại trừ `[data-no-dwell]`). `npm run build` sạch (chạy sau mỗi commit), `openspec validate` pass. Tasks 2.1-2.3 và 4.1-4.3 (manual: mở gate confirm → orb/audio/gesture không đứng hình khi modal mở; dwell trên Remove token/New session không bắn click, chuột vẫn bấm được; dwell trên card/PO option/toggle/close vẫn hoạt động như cũ; dwell trên nút Confirm của gate confirm vẫn bắn) **chưa verify bằng tay trên app thật** — môi trường build này không có `GEMINI_API_KEY`/webcam nên không dựng được app thật để test cử chỉ; cần người dùng chạy qua trước khi archive. 17/21 tasks xong (4 manual còn lại). Bảng "mục nhỏ khác" còn treo: webcam opt-in, camera-error-stuck, barge-in flush race, output-context leak, stale-closure comment — vẫn mở trước Wave 2 (BUG I.2–I.5) |
| 2026-07-22 | Manual verify 2.1-2.3 và 4.1-4.3 xác nhận đạt | Người dùng đã chạy qua app thật: gate confirm mở ra là modal in-app (không phải dialog native), orb tiếp tục render và audio tiếp tục phát trong lúc modal mở, Confirm/Cancel/Escape/backdrop đều đúng hành vi; dwell trên Remove token và New session không bắn click (chuột vẫn bấm được), dwell trên card/PO option/toggle/close vẫn hoạt động như cũ, dwell trên nút Confirm của gate confirm vẫn bắn. `harden-interaction-safety` 21/21 tasks xong, sẵn sàng archive |
| 2026-07-22 | **Webcam opt-in + camera-error-stuck xong** — `fix-gesture-camera-lifecycle`, hai commit | **Item 5:** gesture control trở thành một preference **opt-in, persisted** (`HAND_STORAGE_KEY = "iris.handControlEnabled"`, mặc định **off**) mirror đúng pattern `soundsEnabled`/`cameraDeviceId` (`App.tsx`); `handControl` state khởi tạo từ `loadHandEnabled()`, `start()` không còn `setHandControl(true)` vô điều kiện, và toggle đổi thành `toggleHand()` ghi giá trị mới vào `localStorage` (parallel `toggleSounds`); `stop()` giữ nguyên `setHandControl(false)` (chỉ tắt camera đang chạy, không đụng preference). Kết quả: launch/connect mặc định **không** bật camera/tải MediaPipe; bật một lần thì persist qua session sau. Spec delta: `two-hand-gestures` — một requirement **ADDED** ("Gesture control is an opt-in, persisted preference"). **Item 6:** `useHandControl.ts`'s `setup()` giờ `setError(null)` ngay sau khi `getUserMedia`/`video.play()` thành công và `!cancelled` (không clear ở đầu `setup()` để tránh nhấp nháy lỗi→hết→lỗi khi retry vẫn fail), và nhánh `!enabled` cũng `setError(null)` để lỗi không dính lại khi tắt gesture control. Đường `catch` (báo lỗi) và "không silent retry / không silent fallback thiết bị" giữ nguyên byte-for-byte. Spec delta: `two-hand-gestures` — requirement "Device-selectable camera acquisition" **MODIFIED** (thêm: acquire thành công hoặc disable thì clear lỗi đã báo trước đó), kèm scenario "Error clears when a re-acquire succeeds". `npm run build` sạch (chạy sau mỗi commit), `openspec validate` pass. Tasks 2.1-2.3 và 4.1-4.3 (manual: fresh profile → đèn camera tắt lúc launch, không fetch MediaPipe; bật toggle → camera lên, relaunch → vẫn bật không cần bật lại; tắt toggle → camera tắt, relaunch → vẫn tắt; giữ camera bằng app khác rồi bật gesture → báo lỗi, gestures không dùng được; giải phóng camera rồi bật lại → lỗi biến mất; tắt gesture control lúc đang báo lỗi → lỗi biến mất) **chưa verify bằng tay trên app thật** — cần webcam thật và một app khác giữ device để test đường lỗi; cần người dùng chạy qua trước khi archive. 17/21 tasks xong (4 manual còn lại). Bảng "mục nhỏ khác" còn treo: barge-in flush race, output-context leak, stale-closure comment — vẫn mở trước Wave 2 (BUG I.2–I.5) |
| 2026-07-22 | Manual verify 2.1-2.3 và 4.1-4.3 xác nhận đạt | Người dùng đã chạy qua app thật: fresh profile → đèn camera tắt lúc launch, không fetch MediaPipe; bật toggle → camera lên và gestures hoạt động, relaunch → vẫn bật không cần bật lại; tắt toggle → camera tắt, relaunch → vẫn tắt; giữ camera bằng app khác rồi bật gesture → báo lỗi đúng, gestures không dùng được; giải phóng camera rồi bật lại → lỗi biến mất; tắt gesture control lúc đang báo lỗi → lỗi biến mất. `fix-gesture-camera-lifecycle` 21/21 tasks xong, sẵn sàng archive |
| 2026-07-22 | **Barge-in leak + output-context leak xong** — `fix-audio-playback-lifecycle`, hai commit | **Item 7:** `flushEpochRef = useRef(0)` (`useAudioPipeline.ts`); `flushPlayback` tăng epoch cùng lúc dừng mọi source và reset `playbackTimeRef`; `playGeminiAudio` chụp `epoch` lúc vào hàm, và ngay trước `source.start` bail (`source.disconnect()`, không tiến `playbackTimeRef`) nếu epoch đã đổi — một cancellation token phủ mọi `await` (kể cả `context.resume()`), và tự động phủ luôn `stop()` giữa chừng vì `stop()` gọi `flushPlayback()`. `source.start` bọc thêm try/catch làm lưới an toàn dự phòng. **Item 8:** `stop()` sau `flushPlayback()` giờ `await outputContextRef.current?.close().catch(() => undefined)` rồi null `outputContextRef`/`outputAnalyserRef` và reset `playbackTimeRef`; tái tạo ở lần phát kế tiếp đã có sẵn (`?? new AudioContext()` và rebuild analyser khi `analyser.context !== context`), không cần code thêm. Spec mới: `audio-playback` (2 requirement ADDED — ngắt lời hủy audio đang phát/đang lên lịch; stop giải phóng output `AudioContext`). `npm run build` sạch (chạy sau mỗi commit), `openspec validate` pass. Tasks 2.1-2.3 và 4.1-4.3 (manual: ngắt lời giữa chừng → dừng ngay, không mẩu thừa; ngắt lời đúng lúc context đang resume → vẫn không rò rỉ chunk; sau ngắt lời, phản hồi mới phát bình thường trên timeline mới; stop → output context đóng, OS không còn giữ audio output; start lại → phát bình thường với context/analyser mới; lặp lại stop/start vài lần → không rò rỉ/tích luỹ context) **đã được người dùng verify bằng tay trên app thật, đạt cả sáu**. **19/19 tasks xong.** Renderer row còn treo duy nhất: `handleSidecarEvent` stale-closure comment (rides với một change sau); kế tiếp Wave 2 (BUG I.2–I.5) |
| 2026-07-22 | **BUG I.2 xong** — `make-po-turns-cancellable` (chưa archive) | `stop()` (`run-queue.mjs`) hết là no-op cho nhánh active-không-child (PO turn): giờ `run.status = CANCELLED; cancelRun?.(run); return run.status;` — vẫn **không** gọi `finalize()` tại chỗ, giữ đúng bất biến "slot chỉ giải phóng ở một chỗ" (mirror nhánh DEV `killWithEscalation`). `createRunQueue` nhận thêm dependency injected `cancelRun` (song song `startRun`, optional qua `?.` để không phá caller cũ). `po-session.mjs` thêm `cancelPoTurn(state)`: set `state.endReason = { kind: "cancelled" }`, đóng channel, `query.return()` — đúng thân teardown `closePoSession` dùng, chỉ khác `endReason.kind`; **không** xóa session id đã lưu trên đĩa (`workstream.agent_sessions.po`) nên turn PO kế tiếp `resume` đúng conversation cũ — chỉ mất phần việc dở của turn bị hủy. `main.mjs` nối dây: `cancelRun` tra `getPoSessionState(run.workstream_id)` rồi gọi `cancelPoTurn`, route theo workstream, no-op nếu không có session sống; `startPoRun`'s settle `.catch` thêm nhánh `error?.poEndReason?.kind === "cancelled"` → `finalize(CANCELLED, "Run was stopped before completion.")`, song song nhánh `"teardown"` đã có. Nhánh DEV (`child` khác null) và nhánh queued không đổi. Spec delta: `run-execution-queue` — một requirement **MODIFIED** ("Stopping a run"): PO-stop từ no-op cố ý chuyển sang cancel thật, scenario "Stop an active PO turn" viết lại theo hành vi mới; DEV/queued/escalation/consistency giữ nguyên. Test mới: 2 test trong `run-queue.test.mjs` (tổng 49 test, tất cả pass) — `stop()` gọi đúng `cancelRun` injected, đánh dấu `cancelled`, không tự finalize/không tự giải phóng slot; sau khi (fake) transport finalize thì slot mới giải phóng và run kế tiếp start đúng một lần, không double-start. `npm test` 49/49 pass, `npm run build` sạch, `openspec validate` pass. Tasks 5.1-5.3 (manual: bấm stop một PO turn dài thật qua giọng nói/UI → run đạt `cancelled`, slot giải phóng ngay không cần đợi watchdog; run xếp hàng phía sau start đúng một lần; gửi turn PO tiếp theo trong cùng workstream → tiếp tục đúng conversation cũ nhờ session id được giữ) **chưa verify bằng tay trên app thật — cần người dùng chạy qua trước khi archive**. 18/21 tasks xong (3 manual còn lại). Đóng nốt I.2 của Wave 2; còn I.3 (`detached` + kill process group) và I.4 (`before-quit` `preventDefault` + await teardown), cộng mục nhỏ `handleSidecarEvent` stale-closure comment |
| 2026-07-22 | **BUG I.3 + I.4 xong** — `bound-shutdown-teardown`, hai commit | **I.3:** `createRunQueue` nhận thêm dependency injected `killChild(child, signal)` (`run-queue.mjs`, song song `cancelRun`, mặc định `(child, signal) => child.kill(signal)` — giữ nguyên hành vi cho caller không inject); `killWithEscalation` gọi `killChild(run.child, "SIGTERM")` rồi `killChild(run.child, "SIGKILL")` sau grace 5s thay vì gọi `run.child.kill` trực tiếp — không đổi gì khác ở luồng escalation/finalize. DEV `spawn` (`main.mjs`) thêm `detached: true` (không `unref()` — parent vẫn quản lý child) nên là process-group leader trên POSIX. `main.mjs` cung cấp `killChild` cross-platform: POSIX `process.kill(-child.pid, signal)` bọc try/catch (fallback `child.kill(signal)` nếu group đã mất/thiếu `pid`); Windows `spawn("taskkill", ["/pid", pid, "/T", "/F"])` (`signal` chỉ tham khảo) — pass vào `createRunQueue(...)` cùng `startRun`/`cancelRun`. Verify độc lập bằng script Node đứng ngoài Electron: spawn `bash -c "sleep 60 & wait"` với `detached: true`, gọi `killChild(child, "SIGTERM")` — `pgrep` xác nhận cả bash cha lẫn `sleep` con đều biến mất, không mồ côi. **I.4:** `closePoSession`/`closeAllPoSessions` (`po-session.mjs`) giờ trả về promise teardown (`state.query?.return?.()` / `Promise.all(...)`) — caller fire-and-forget cũ (reset, đổi workstream) bỏ qua giá trị trả về, hành vi không đổi. `before-quit` (`main.mjs`) viết lại: guard module-level `shuttingDown` (bỏ qua tín hiệu quit lặp lại), `event.preventDefault()`, rồi `Promise.race([shutdownTeardown(), deadline]).finally(() => app.exit(0))`; `shutdownTeardown()` await `stopLive()`, group-kill mọi DEV child còn sống qua `killChild`, rồi `await closeAllPoSessions()`; deadline mới `IRIS_SHUTDOWN_DEADLINE_MS` (mặc định 8000ms, thêm vào `.env.example`) timer `unref()`'d. `will-quit`'s `globalShortcut.unregisterAll()` không đổi. Spec delta: `run-execution-queue` — MODIFIED requirement "Stopping a run" (escalation giờ mô tả group-kill qua injected hook); capability mới `app-shutdown` (2 requirement ADDED). Đóng drift với `po-live-session`'s "Live session ends cleanly on app shutdown" — không cần sửa spec đó, code giờ tuân theo. Test: sửa test escalation cũ sang spy trên `killChild` injected + 1 test mới xác nhận default `killChild` fallback về `child.kill` khi không inject (`npm test` toàn repo 47 → 48, tất cả pass); `npm run build`, `npm run package:mac` (unpacked, chưa code-sign — môi trường build không có Developer ID) đều sạch; `openspec validate` pass. Tasks 6.1-6.3, 6.5 (manual: dừng một DEV run thật đang spawn shell con qua Iris và xác nhận qua `ps`; quit app khi đang có DEV run + PO turn sống; `IRIS_SHUTDOWN_DEADLINE_MS` thấp + teardown treo; Windows `taskkill /T /F`) **đã được người dùng verify bằng tay trên app thật, đạt**. **25/25 tasks xong.** Đóng nốt I.3+I.4 của Wave 2; còn I.5 (đã thoả mãn sẵn bởi once-guard `finalize()`, cần verify rồi đóng, không cần code mới) và mục nhỏ `handleSidecarEvent` stale-closure comment |
| 2026-07-22 | Manual verify 6.1-6.3, 6.5 xác nhận đạt | Người dùng đã chạy qua app thật. `bound-shutdown-teardown` 25/25 tasks xong, sẵn sàng archive |
| 2026-07-22 | **I.5 + stale closure xong** — `harden-slot-release-and-event-subscription` (chưa archive) | **I.5:** `finalize()` (`run-queue.mjs`) giữ nguyên mutation record + `emit`/`onFinalized` vô điều kiện (run luôn đạt terminal và thông báo); `clearIdleTimer()`/`idleSuspended = false`/`dequeueNext()` chuyển vào guard `if (active === runId)` — finalize nhắm vào một run không giữ slot giờ **không thể** xoá watchdog hay cướp slot của run đang active. Không caller nào hôm nay finalize một run không-active nên hành vi byte-identical cho mọi caller hiện có; guard chỉ đóng lỗ hổng tiềm ẩn. **Stale closure:** `sidecarHandlerRef = useRef(handleSidecarEvent)` + `useEffect` không-dependency refresh ref mỗi render (`App.tsx`); subscription trong effect `[hasBridge]` (lifecycle đăng ký giữ nguyên, không churn native listener) đổi sang dispatch qua `sidecarHandlerRef.current(event)` — comment giải thích tại sao. Spec delta: `run-execution-queue` — một requirement **MODIFIED** ("A run finalizes exactly once": mệnh đề slot-release giờ giới hạn side-effect vào đúng slot-holder). Test mới: 1 test trong `run-queue.test.mjs` (tổng 49 test, tất cả pass) — finalize một run đang queued (chưa từng giữ slot) đạt terminal + emit đúng một lần, nhưng run active vẫn giữ slot, idle timer không bị clear (verify bằng cách cho watchdog hết hạn sau đó vẫn finalize đúng run active), và không run nào bị start thêm; test được xác nhận **fail đúng như dự đoán** khi chạy tạm lại trên code cũ (trước fix), rồi pass sau khi áp guard. `npm test` 49/49 pass, `npm run build` sạch, `openspec validate` pass. Tasks 4.2-4.3 (manual smoke: sidecar events vẫn cập nhật UI qua ref; submit task khi đang chạy → task xếp hàng start đúng một lần khi active xong) **chưa verify bằng tay trên app thật** — môi trường build này không có `GEMINI_API_KEY`/`CLAUDE_CODE_OAUTH_TOKEN` nên không dựng được phiên voice/PO thật để chạy qua UI; đã xác nhận app boot sạch (`npm run dev`, không crash renderer/main) như một smoke tối thiểu. 15/17 tasks xong (2 manual còn lại). **Đóng nốt đuôi Wave 2** (I.1-I.5 đều xong). Đợt còn lại theo kế hoạch: **Wave 3** — refactor session store (`schemaVersion` cho `claude-sessions.json`) + sidecar-event reducer — chưa bắt đầu |
| 2026-07-23 | Manual verify 4.2-4.3 xác nhận đạt | Người dùng đã chạy qua app thật: sidecar events vẫn cập nhật UI đúng qua ref dispatch (task cards, session list, trạng thái gemini/claude, PO question banner); submit task khi đang có run chạy → task xếp hàng start đúng một lần khi active xong, không double-start. `harden-slot-release-and-event-subscription` 17/17 tasks xong, sẵn sàng archive |
