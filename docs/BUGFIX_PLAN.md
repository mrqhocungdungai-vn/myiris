...
| I | Không có watchdog / PO không hủy được / subprocess mồ côi | 🟠 Design | `run-queue.mjs`, `main.mjs` | lớn | [~] I.1, I.2, I.3, I.4 xong — I.5 còn |
...
- `finalize` idempotent với run không-active (xem BUG K) — **I.5, còn treo**: đã thoả mãn sẵn bởi once-guard của `finalize()` (`run.finalized`), cần xác nhận lại rồi đóng, không cần code mới.
