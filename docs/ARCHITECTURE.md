...
- **`src/hooks/useWakeWord.ts`** — the on-device "Hey Iris" listener, armed only while Iris is asleep. Owns the capture stream and the two model pipelines that read it; everything decidable without audio is factored out beside it. A wake needs **two independent signals to agree** (`wake-sleep-voice` spec): the openWakeWord phrase chain (mel → embedding → classifier, `public/wakeword/`), sustained over N consecutive evaluations 200 ms apart; **and** Silero VAD confirming a human voice within 1.5 s either side of that detection. The second signal exists because the phrase classifier was trained on synthesized speech and scores confidently on non-speech sound — an error inside the model, which no threshold on its own output can detect. It fails open: if the VAD cannot load or throws, the decision reverts to the phrase signal alone, because an app that cannot wake is worse than one that occasionally wakes when it shouldn't.
  - **`src/lib/wake-gate.ts`** — the fire/hold/reset decision as a pure function of scores, confirmation events and timestamps. A phrase detection arriving with no voice yet is *held*, not discarded, so confirmation a moment later still wakes. Loads no model and touches no browser global, which is what keeps every wake scenario exercisable by `npm test`.
  - **`src/lib/speech-confirmation.ts`** — the VAD subsystem: its own load (deliberately *not* joined to the phrase models' `Promise.all`, where a failure would take the whole listener down), a bounded frame backlog, and degrade-to-inert on error.
  - **`src/lib/silero-vad.ts`** / **`src/lib/frame-accumulator.ts`** — the recurrent model's tensors and state threading, and the re-chunking that feeds it. The two pipelines have incompatible audio contracts: the phrase chain re-reads a 2 s ring buffer every 200 ms, while Silero needs contiguous, non-overlapping 512-sample frames with its state carried call to call — so the VAD gets its own accumulator off the same `onaudioprocess` callback rather than a second capture.
  - **`src/lib/onnx-runtime.ts`** — the shared `onnxruntime-web` bootstrap (vendored WASM paths, single-threaded) both pipelines load through.
